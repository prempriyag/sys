...
async def insert(request: CombinedNameRequest, http_request: Request, db: Session = Depends(get_db)):
    try:
...
async def update(request: CombinedNameUpdateRequest, http_request: Request, db: Session = Depends(get_db)):
    try:
        if not request.CombinedWords:
            return {"status": 0, "message": "Please enter combined words", "refresh": False, "modal_close": False}
        combined_words = request.CombinedWords.strip().capitalize()
