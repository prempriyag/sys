...
async def insert(request: SuffixRequest, http_request: Request, db: Session = Depends(get_db)):
    try:
...
async def update(request: SuffixUpdateRequest, http_request: Request, db: Session = Depends(get_db)):
    try:
        if not request.Suffix:
            return {"status": 0, "message": "Please enter a suffix", "refresh": False, "modal_close": False}
        suffix = request.Suffix.strip().capitalize()
