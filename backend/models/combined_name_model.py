...
class CombinedNameModel:
    ...
    def build_search_conditions(request_data: Dict[str, Any]) -> str:
        ...
        if search_value:
            ...
            search_conditions.append(f"""(lower(CombinedWords) like '%{search_lower}%' or 
        ...
        if columns:
            for col in columns:
                ...
                if col_search_value:
                    ...
                    if col_data == "CombinedWords":
                        search_conditions.append(f"lower(CombinedWords) like '%{col_search_lower}%'")
    ...
    @staticmethod
    def get_combined_name_data(db: Session, request_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            search_query = CombinedNameModel.build_search_conditions(request_data)
            order_by_column = CombinedNameModel.get_order_by_column(column_name)
            order_by_clause = f"{order_by_column} {column_dir.upper()}"
            where_clause = search_query if search_query else "1=1"
            count_query = text(f"SELECT count(*) as allcount FROM {TBL_CombinedName} WITH(NOLOCK) WHERE {where_clause}")
            ...
            data_query_sql = f"""
                SELECT ID, CombinedWords, Updated_By, Updated_on
                FROM {TBL_CombinedName} WITH(NOLOCK)
                WHERE {where_clause}
            ...
            for record in records:
                ...
                data_row = {
                    ...
                    "CombinedWords": record_dict.get("CombinedWords", ""),
