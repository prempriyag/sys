"""Grade Mapping Model - Equivalent Grades"""
...
class GradeMappingModel:
    ...
    @staticmethod
    def get_grade_mapping_data(db: Session, request_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            search_query = GradeMappingModel.build_search_conditions(request_data)
            order_by_column = GradeMappingModel.get_order_by_column(column_name)
            order_by_clause = f"{order_by_column} {column_dir.upper()}"
            where_clause = search_query if search_query else "1=1"
            count_query = text(f"SELECT count(*) as allcount FROM {TBL_GRADE} WITH(NOLOCK) WHERE {where_clause}")
            ...
            data_query_sql = f"""
                SELECT ID, EQUIVALENT_GRADE, TRANSCRIPT_GRADE, UPDATED_BY, UPDATED_ON
                FROM {TBL_GRADE} WITH(NOLOCK)
                WHERE {where_clause}
            ...
            for record in records:
                ...
                data_row = {
                    ...
                    "EQUIVALENT_GRADE": record_dict.get("EQUIVALENT_GRADE", ""),
                    "TRANSCRIPT_GRADE": record_dict.get("TRANSCRIPT_GRADE", ""),
