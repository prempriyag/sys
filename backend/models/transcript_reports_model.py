...
class TranscriptReportsModel:
    ...
    def build_search_conditions(
        ...
        if search_value:
            ...
            search_conditions.append(f"""( k.BATCH_ID like '%{search_safe}%' or
                ...
                lower(k.STATUS_SLATE) like '%{search_lower}%' or 
                lower(k.SLATE_REF_NUMBER) like '%{search_lower}%' or 
                lower(k.STATUS_SLATE_UPLOAD) like '%{search_lower}%' or 
        ...
        if field_type and field_name:
            ...
            elif field_type == "STATUS_SLATE":
                search_conditions.append(f"k.STATUS_SLATE = '{check_special_name(field_name)}'")
            elif field_type == "SLATE_REF_NUMBER":
                search_conditions.append(f"k.SLATE_REF_NUMBER = '{check_special_name(field_name)}'")
            elif field_type == "STATUS_SLATE_UPLOAD":
                search_conditions.append(f"k.STATUS_SLATE_UPLOAD = '{check_special_name(field_name)}'")
    ...
    def get_reports_data(
        ...
        try:
            ...
            test_sql = f"SELECT COUNT(*) as test_count FROM {TBL_KICKOUT} WITH(NOLOCK) WHERE PROJECT_ID = {COLLEGE_PROJECT_ID}"
            try:
                test_result = db.execute(text(test_sql)).fetchone()
                test_count = test_result.test_count if test_result else 0
                print(f"[TranscriptReports] TEST: Total records with PROJECT_ID={COLLEGE_PROJECT_ID}: {test_count}")
            ...
            search_query = TranscriptReportsModel.build_search_conditions(request_data)
            ...
            logger.info(f"[TranscriptReports] PROJECT_ID: {COLLEGE_PROJECT_ID}")
            ...
            # Get order by column
            order_by = TranscriptReportsModel.get_order_by_column(column_name)
            ...
            for record in records:
                ...
                error_reason = TranscriptReportsModel.format_error_reason(
                    record_dict.get("ERROR_REASON")
                )

                # Calculate STATUS_BANNER (matches CI3 lines 784-790)
                status_banner = TranscriptReportsModel.calculate_status_banner(record_dict)
                ...
                if institution_id == "000000":
                    ...
                    institution_name = TranscriptReportsModel.get_institution_name_for_000000(db, batch_id)
                    institution_name_formatted = institution_name.lower() if institution_name else ""
                else:
                    institution_name_formatted = institution_name.lower() if institution_name else ""

                # Format dates
                degree_received_date = TranscriptReportsModel.format_date(
                    record_dict.get("DEGREE_RECEIVED_DATE")
                )
                second_degree_received_date = TranscriptReportsModel.format_date(
