...
class TransferGradesMappingModel:
    ...
    @staticmethod
    def get_transfer_grades_mapping_data(db: Session, request_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            search_query = TransferGradesMappingModel.build_search_conditions(request_data)
            order_by_column = TransferGradesMappingModel.get_order_by_column(column_name)
            order_by_clause = f"{order_by_column} {column_dir.upper()}"
            where_clause = search_query if search_query else "1=1"
            count_query = text(f"SELECT count(*) as allcount FROM {TBL_TRANSFER_GRADES_MAPPING} WITH(NOLOCK) WHERE {where_clause}")
            ...
            data_query_sql = f"""
                SELECT ID, TRANSFER_GRADE, UPDATED_BY, UPDATED_ON
                FROM {TBL_TRANSFER_GRADES_MAPPING} WITH(NOLOCK)
                WHERE {where_clause}
