/**
 * Bulk Electoral Roll page - PDF extraction + insert to voter_data
 * Isolated module: uses modules/bulk-electoral-roll/api.ts
 */
...
import { bulkElectoralRollWithProgress } from './api';
...
const BulkElectoralRollPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.detail ?? e?.message ?? 'Bulk processing failed.';
    ...
    <PageContainer>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          <strong>Production:</strong> Enter server folder path — extracts all pages, box_id per PDF, insert then move. <strong>Or</strong> select folder / pick PDF files to upload. Fill State, Year, District, Constituency for extracted subfolders.
        </p>

        <div className="space-y-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">State (optional)</label>
              <input
                type="text"
                value={state}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">District (optional)</label>
              <input
                type="text"
                value={district}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Year (optional)</label>
              <input
                type="text"
                value={year}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Server folder path (production)</label>
            <input
              type="text"
              value={folderPath}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Or select folder (browser)</label>
            <button
              type="button"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Or pick PDF files</label>
            <input
              ref={fileInputRef}
              type="file"
            ...
            {selectedFiles.length > 0 && !selectedFolderName && (
              <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{selectedFiles.length} PDF(s) selected</p>
            )}
          </div>
        </div>
        ...
        {result && (
          <div className="mt-6 p-4 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900">
            ...
            <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
              <li><strong>Total found:</strong> {result.total_found}</li>
              <li><strong>Inserted:</strong> {result.inserted}</li>
              <li><strong>Duplicates skipped:</strong> {result.duplicates_skipped}</li>
              <li><strong>Invalid EPIC count:</strong> {result.invalid_epic_count}</li>
              {result.pdf_count != null && <li><strong>PDFs processed:</strong> {result.pdf_count}</li>}
              {result.moved_count != null && <li><strong>PDFs moved:</strong> {result.moved_count}</li>}
              {result.extracted_folder && (
                <li><strong>Extracted folder:</strong> <span className="font-mono text-xs break-all">{result.extracted_folder}</span></li>
              )}
            ...
            {result.errors && result.errors.length > 0 && (
              <div className="mt-3 p-3 border border-red-300 dark:border-red-700 rounded-lg bg-red-50 dark:bg-red-900/20">
                <p className="text-sm font-medium text-red-800 dark:text-red-200">Insert failed for some PDFs:</p>
                <ul className="mt-1 text-xs text-red-700 dark:text-red-300 font-mono space-y-1">
                  {result.errors.slice(0, 10).map((err, i) => (
                    <li key={i}>{err}</li>
                  ))}
                </ul>
                {result.errors.length > 10 && <p className="mt-1 text-xs text-red-600 dark:text-red-400">… and {result.errors.length - 10} more</p>}
                {(result.inserted === 0 && result.errors.some((e) => /column|does not exist|relation|pdf_name|box_id/i.test(e))) && (
                  <p className="mt-2 text-sm font-medium text-red-800 dark:text-red-200">
                    Fix: Run <code className="bg-red-100 dark:bg-red-900/40 px-1 rounded">alembic upgrade head</code> in backend
                  </p>
                )}
              </div>
            )}
            {result.invalid_epics && result.invalid_epics.length > 0 && (
              <div className="mt-3">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Invalid EPICs (sample):</p>
                <p className="text-xs text-gray-600 dark:text-gray-400 font-mono mt-1 break-all">{result.invalid_epics.join(', ')}</p>
              </div>
            )}
          </div>
        )}
