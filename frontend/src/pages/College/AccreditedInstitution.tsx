...
export default function AccreditedInstitution() {
  ...
  const handleEdit = async (id: number) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/accreditedinstitution/get`, {
        method: "POST",
      ...
      if (data.Id) {
        setFormData({
          INSTITUTION_ID: data.INSTITUTION_ID || "", EFFECTIVE_START_TERM: data.EFFECTIVE_START_TERM || "",
      ...
      setMessage({ type: "error", text: error.message || "Error loading accredited institution data" });
    }
  };

  const handleDelete = async (id: number) => {
    if (!window.confirm("Are you sure you want to delete this accredited institution?")) return;
    try {
      const response = await fetch(`${API_BASE_URL}/api/accreditedinstitution/delete`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("auth_token")}` },
        body: JSON.stringify({ id }),
      });
      ...
      if (data.status === "Success") {
        setMessage({ type: "success", text: "Accredited institution deleted successfully" });
        setRefreshTrigger((prev) => prev + 1);
      } else {
        setMessage({ type: "error", text: "Error deleting accredited institution" });
      }
    } catch (error: any) {
      setMessage({ type: "error", text: error.message || "Error deleting accredited institution" });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    try {
      ...
      if (data.status === 1) {
        ...
        setMessage({ type: "error", text: data.message || "Error saving accredited institution" });
      }
    } catch (error: any) {
      setMessage({ type: "error", text: error.message || "Error saving accredited institution" });
    }
  };
      ...
      <PageContainer>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Add Accredited Institution</h3>
              ...
              <form onSubmit={(e) => handleSubmit(e, false)}>
                ...
                <div className="mb-4"><label className="block text-sm font-medium mb-2">Effective Start Term *</label><input type="text" value={formData.EFFECTIVE_START_TERM} onChange={(e) => setFormData({ ...formData, EFFECTIVE_START_TERM: e.target.value })} required className="w-full px-4 py-2 border rounded-lg" /></div>
                <div className="mb-4"><label className="block text-sm font-medium mb-2">Effective End Term *</label><input type="text" value={formData.EFFECTIVE_END_TERM} onChange={(e) => setFormData({ ...formData, EFFECTIVE_END_TERM: e.target.value })} required className="w-full px-4 py-2 border rounded-lg" /></div>
        ...
        {showEditModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Edit Accredited Institution</h3>
