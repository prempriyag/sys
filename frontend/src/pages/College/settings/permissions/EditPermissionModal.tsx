...
export default function EditPermissionModal({
      ...
      const fetchPermission = async () => {
        ...
        try {
          const response = await api.post("/api/permissions/getpermission", {
            id: permissionId,
            key: isParent ? null : undefined,
          });

          console.log("Permission response:", response); // Debug log

          // Handle both wrapped and direct response formats
          // The API might return {status: 1, data: {...}} or just {...}
          let data;
          ...
          // Determine if this is a parent permission
          // per_type can be "parent" (string) or a number (parent ID)
          const perType = data.per_type || data.TYPE;
          const isParentType = perType === "parent" || String(perType).toLowerCase() === "parent";

          console.log("Parsed permission data:", {
            id: data.id || data.ID,
            per_name: data.per_name || data.PERMISSION_NAME,
            per_key: data.per_key || data.PERMISSION_KEY,
            per_type: perType,
            isParentType
          });

          setFormData({
            id: data.id || data.ID || permissionId,
            permission_name: data.per_name || data.PERMISSION_NAME || "",
            permission_key: data.per_key || data.PERMISSION_KEY || "",
            type: isParentType ? "parent" : (perType || "parent"),
          });
          setShowKeyField(!isParentType);
        } catch (err: any) {
  ...
  }, [isOpen, permissionId, isParent, alerterror]);
  ...
  const handleTypeChange = (value: string) => {
    const isParentType = value === "parent";
    setFormData({
      ...formData,
      type: value,
      permission_key: isParentType ? "" : formData.permission_key,
    });
    setShowKeyField(!isParentType);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.post(`/api/permissions/update/${formData.id}`, {
      ...
      if (response.status === 1) {
        const successMessage = response.message || "Permission updated successfully";
        alertsuccess(successMessage);
        onSuccess();
        onClose();
      } else {
        const errorMessage = response.message || "Failed to update permission";
        alerterror(errorMessage);
      }
    } catch (err: any) {
        ...
        {loadingPermission ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-500">Loading permission data...</p>
            </div>
          </div>
        ) : (
