...
export default function AcceptedGradesMapping() {
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      setMessage({
        type: "error",
        text: error.response?.data?.detail || error.message || "Error loading grade data",
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.response?.data?.detail || error.message || "Error deleting grade",
      });
      setShowDeleteConfirmModal(false);
      setGradeToDelete(null);
      setTimeout(() => setMessage(null), 5000);
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.response?.data?.detail || error.message || "Error saving grade",
      });
      setTimeout(() => setMessage(null), 5000);
    } finally {
      setLoading(false);
      ...
      <PageContainer>
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasAddPermission && (
              <Button onClick={handleAdd} startIcon={<PlusIcon className="w-5 h-5" />}>
                Add Grade
              </Button>
            )}
          </div>
