...
export default function CombinedName() {
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      setMessage({ type: "error", text: error.response?.data?.detail || error.message || "Error loading combined name data" });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMessage({ type: "error", text: error.response?.data?.detail || error.message || "Error deleting combined name" });
      setShowDeleteConfirmModal(false);
      setCombinedNameToDelete(null);
      setTimeout(() => setMessage(null), 5000);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    try {
      ...
      if (success) {
        ...
        setMessage({ type: "error", text: messageText || "Error saving combined name" });
        setTimeout(() => setMessage(null), 5000);
      }
    } catch (error: any) {
      console.error("Save combined name error:", error);
      setMessage({ type: "error", text: error.response?.data?.detail || error.message || "Error saving combined name" });
      setTimeout(() => setMessage(null), 5000);
    } finally {
      setLoading(false);
    }
