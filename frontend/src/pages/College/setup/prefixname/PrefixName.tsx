...
export default function PrefixName() {
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      alerterror(error.response?.data?.detail || error.message || "Error loading prefix data");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alerterror(error.response?.data?.detail || error.message || "Error deleting prefix");
      setShowDeleteConfirmModal(false);
      setPrefixToDelete(null);
    } finally {
      setIsDeleting(false);
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    try {
      ...
      if (success) {
        ...
        alerterror(messageText || "Error saving prefix");
      }
    } catch (error: any) {
      console.error("Save prefix error:", error);
      alerterror(error.response?.data?.detail || error.message || "Error saving prefix");
    } finally {
      setLoading(false);
    }
  };
