...
export default function SuffixName() {
  ...
  const handleEdit = async (id: number) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/suffixname/get`, {
        method: "POST",
      ...
      setMessage({ type: "error", text: error.message || "Error loading suffix data" });
    }
  };

  const handleDelete = async (id: number) => {
    if (!window.confirm("Are you sure you want to delete this suffix?")) return;
    try {
      const response = await fetch(`${API_BASE_URL}/api/suffixname/delete`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("auth_token")}` },
        body: JSON.stringify({ id }),
      });
      ...
      if (data.status === "Success") {
        setMessage({ type: "success", text: "Suffix deleted successfully" });
        setRefreshTrigger((prev) => prev + 1);
      } else {
        setMessage({ type: "error", text: "Error deleting suffix" });
      }
    } catch (error: any) {
      setMessage({ type: "error", text: error.message || "Error deleting suffix" });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    try {
      ...
      if (data.status === 1) {
        ...
        setMessage({ type: "error", text: data.message || "Error saving suffix" });
      }
    } catch (error: any) {
      setMessage({ type: "error", text: error.message || "Error saving suffix" });
    }
  };
      ...
      <PageContainer>
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasAddPermission && <Button onClick={handleAdd} startIcon={<PlusIcon className="w-5 h-5" />}>Add Suffix</Button>}
          </div>
        </div>
        {message && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Add Suffix</h3>
                <button onClick={() => setShowAddModal(false)} className="text-gray-500 hover:text-gray-700">×</button>
              </div>
              <form onSubmit={(e) => handleSubmit(e, false)}>
                <div className="mb-4"><label className="block text-sm font-medium mb-2">Suffix *</label><input type="text" value={formData.Suffix} onChange={(e) => setFormData({ ...formData, Suffix: e.target.value })} required className="w-full px-4 py-2 border rounded-lg" /></div>
        ...
        {showEditModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Edit Suffix</h3>
