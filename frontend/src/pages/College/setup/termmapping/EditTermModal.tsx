...
export default function EditTermModal({
  ...
  useEffect(() => {
    if (isOpen && termId) {
      const fetchData = async () => {
        setLoadingTerm(true);
        try {
          const response = await api.post(`${API_BASE_URL}/api/termmapping/get`, { id: termId });
          ...
          if (response && (response.Id || response.id)) {
            const startDate = response.TERM_START ? new Date(response.TERM_START).toISOString().split('T')[0] : "";
            const endDate = response.TERM_END ? new Date(response.TERM_END).toISOString().split('T')[0] : "";
            ...
            setFormData({
              ...
              TERM_END: endDate,
              IS_ACTIVE: response.IS_ACTIVE || "Y",
              GRACE_PERIOD: response.GRACE_PERIOD || 0,
            });
            ...
            alerterror("Failed to load term data: Invalid response format");
          }
        } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await api.post(`${API_BASE_URL}/api/termmapping/update`, {
        Id: termId,
        ...formData,
      });
      
      if (response.status === 1 || response.message?.includes("success")) {
        alertsuccess(response.message || "Term updated successfully");
        onSuccess();
        onClose();
      } else {
        alerterror(response.message || "Failed to update term");
      }
    } catch (err: any) {
      console.error("Update term error:", err);
      alerterror(err.response?.data?.message || err.message || "Failed to update term");
    } finally {
      setLoading(false);
    }
  };
        ...
        {loadingTerm ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-500">Loading term data...</p>
            </div>
          </div>
        ) : (
