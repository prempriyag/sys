...
export default function CollegeAssignedBatches() {
  ...
  const canReassign = hasPermission("college_ocr_data", "update"); // or admin/developer check
  ...
  const handleReassign = async () => {
    ...
    try {
      await api.post(API_ENDPOINTS.OCR_COLLEGE_OCR_REASSIGN_VERIFIER, {
  ...
  return (
    <PageWrapper>
      <PageMeta title="College Assigned Batches | OCR" description="College OCR batches assigned to verifiers" />
      <PageBreadcrumb pageTitle="College Assigned Batches" />

      <PageContainer>
        {/* Filter panel - shown/hidden via toggle */}
        {showFilters && (
          <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900/50">
        ...
        <DataTable
          ...
          ajaxUrl={API_ENDPOINTS.OCR_COLLEGE_OCR_AJAX_VERIFIER_BATCH_LIST}
          ajaxData={ajaxData}
          columns={[
            ...(canReassign
              ? [{
              ...
              render: (data: string) =>
                data ? (
                  <span className="inline-flex items-center whitespace-nowrap">
                    ...
                    <Link
                      to={`/ocrverify/collegeocrbatch?batch_id=${encodeURIComponent(data)}&verify=no`}
