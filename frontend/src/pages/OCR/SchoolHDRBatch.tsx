...
export default function SchoolHDRBatch() {
  ...
  const fetchBatch = async () => {
    try {
      setLoading(true);
      setError(null);
      const url = `${API_ENDPOINTS.OCR_SCHOOL_HDR_BATCH}?batch_id=${encodeURIComponent(batchId)}&verify=${encodeURIComponent(verify)}`;
  ...
  const handleSaveHeader = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.post(API_ENDPOINTS.OCR_SCHOOL_HDR_UPDATE_BATCH_HDR, payload);
  ...
  if (loading) {
    return (
      <PageWrapper>
        <PageMeta title="School HDR Batch | OCR" description="School HDR batch view" />
        <PageBreadcrumb pageTitle="School HDR Batch" />
        <PageContainer>
      ...
      <PageWrapper>
        ...
        <PageContainer>
          <div className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800 p-6 text-center">
            ...
            <Link to="/ocrverify/schoolhdrdata" className="mt-4 inline-block text-brand-600 hover:underline">
              Back to School Header Data
            </Link>
          </div>
        </PageContainer>
      </PageWrapper>
  ...
  return (
    <PageWrapper>
      <PageMeta title={`School HDR Batch: ${batchId} | OCR`} description="School HDR batch view" />
      <PageBreadcrumb pageTitle={`School HDR Batch: ${batchId}`} />

      <PageContainer>
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <Link to="/ocrverify/schoolhdrdata" className="text-brand-600 hover:underline">
            ← School Header Data
          </Link>
          {batch.prev_batch_id && (
            <Link
              to={`/ocrverify/schoolhdrbatch?batch_id=${encodeURIComponent(batch.prev_batch_id)}&verify=${verify}`}
          ...
          {batch.next_batch_id && (
            <Link
              to={`/ocrverify/schoolhdrbatch?batch_id=${encodeURIComponent(batch.next_batch_id)}&verify=${verify}`}
              className="rounded border border-gray-300 bg-white px-3 py-1 text-sm hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:hover:bg-gray-700"
            >
              Next →
            </Link>
            ...
            <div className="overflow-x-auto p-4">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {linedata.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="p-4 text-center text-gray-500">No line data.</td>
                    </tr>
                  ) : (
                    linedata.map((line) => (
