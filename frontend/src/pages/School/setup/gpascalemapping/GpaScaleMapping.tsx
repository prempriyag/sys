...
export default function GpaScaleMapping() {
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      setMessage({ type: "error", text: error.response?.data?.detail || error.message || "Error loading GPA Scale data" });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMessage({ type: "error", text: error.response?.data?.detail || error.message || "Error deleting GPA Scale" });
      setShowDeleteConfirmModal(false);
      setGpaScaleToDelete(null);
      setTimeout(() => setMessage(null), 5000);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent, isEdit: boolean) => {
    ...
    try {
      ...
      if (success) {
        ...
        setMessage({ type: "error", text: messageText || "Error saving GPA Scale" });
        setTimeout(() => setMessage(null), 5000);
      }
    } catch (error: any) {
      console.error("Save GPA Scale error:", error);
      setMessage({ type: "error", text: error.response?.data?.detail || error.message || "Error saving GPA Scale" });
      setTimeout(() => setMessage(null), 5000);
    } finally {
      setLoading(false);
    }
      ...
      <PageContainer>
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasAddPermission && <Button onClick={handleAdd} startIcon={<PlusIcon className="w-5 h-5" />}>Add GPA Scale</Button>}
          </div>
        </div>
